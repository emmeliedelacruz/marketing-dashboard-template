...
- 2026-08-23 (latest): Pause and Duplicate now open the platform's own editor instead of trying to act. Duplicate no longer mints a test ad id from the control's (`<control>_V2` was never issued by any platform) and no longer splits the control ad's real spend to fabricate a creative row. Pause sets no local flag and logs a Completed card, deduped on entity + action + date. Adds `control_ad_id` / `test_ad_id` card fields that take an id or an Ads Manager link, refuse wrong-object links and legacy `_V2` values, and block the save rather than dropping a bad value. The Meta ad filter is read off a real URL and asserted byte-for-byte.
...
- **The strip populates itself.** A card put into Testing by hand carries no ad IDs, and a blank strip on a card that says it is testing something reads as broken. Once a card is past To Test, `resolveUnitAds()` finds the unit's ads in the creative data — matching the card's unit against `ad_set` (or ad `name`), taking the highest-spending ad as the control and, where ad names carry a `_V<n>` variant suffix, the highest of those as the test. Explicit ad IDs written by Duplicate always win over this. Where no test ad exists yet the test column shows an em dash, which is the honest reading: nothing has been duplicated. Auto-matched cards label themselves "matched from ad set", so an inferred pair is never mistaken for a recorded one.
...
- **A test ad is a real row, with a real id.** Once a duplicate has run it appears in the creative array with the **platform's own `ad_id`**, the same `ad_set`, and its own spend — so creative spend still rolls up to the channel total. Never derive the test ad's id from the control's (`<control>_V2` and the like): an id that encodes another id is one no platform ever issued, so the card can never be matched against a real pull. Ad *names* may carry a `_V2` suffix; ids may not.
- **Card actions:** Duplicate (opens the platform's editor — see below) · Complete (→ Completed) · Delete (permanent)
- **Duplicate opens the native editor. It creates nothing.** A browser page cannot call an ad platform API directly — the platforms send no CORS headers, and an access token in the page is a leaked credential — so the click does the three things it honestly can: open Ads Manager filtered to the source ad, write the **control** id onto the card, and set the card to *awaiting an id*. A person makes the copy, leaves it PAUSED, and pastes the new ad's real id back. On a server build the token lives server-side and the server can create the ad for real; see `docs/railway-setup.md`.
- **Never invent a number or an id.** This is the rule the whole feature exists under. An earlier version minted the test ad's id by appending `_V2` to the control's, then **split the control ad's real spend, clicks and conversions in half** to populate a synthetic creative row — altering a real ad's real figures in a dashboard leadership reads. A duplicated ad has no performance yet: it starts empty and shows real numbers on the next pull. An id on a card only ever comes from the platform.
- **Pause opens the native editor too, and logs.** The dashboard does not pause anything. The click opens Ads Manager for that exact ad, ad set or campaign, and writes a Changelog card in **Completed** reading `Paused on <date>. <Platform> <level> ID <id>.` That is the whole value: the trip to the platform and the log entry, which otherwise gets written later or not at all. Set **no** local "paused" flag — the page cannot know whether the pause happened, and the real status arrives with the next refresh. Dedupe on entity + action + date so pausing, unpausing and pausing again in one sitting does not stack identical cards. Leave unpause alone unless asked for.
- **Read every deep-link URL off a real session. Do not infer it.** Meta's verified ad-level filter is `filter_set=SEARCH_BY_ADGROUP_ID-STRING\u001EEQUAL\u001E"<ad_id>"`, run through `encodeURIComponent`. Two things a guess gets wrong: `\u001E` is an ASCII record separator delimiting field / operator / value, not `&` with bracketed indices; and **ADGROUP is Meta's legacy name for an AD**, not an ad set. An invalid `filter_set` is accepted silently and narrows nothing. For any level or platform whose filter key you have not read off a real URL, use `selected_<level>_ids`, which highlights the row inside the full list — a working select beats an unverified filter. **Assert the generated URL byte-for-byte** against the one the platform produced, so a future edit that breaks the encoding fails the suite instead of quietly opening an unfiltered table.
- **Deep-link only to an entity the data can name.** Ad rows carry `ad_id`; ad set and campaign rows need an `id` column for the link to select them. Without one, open the account and *say so in the confirm* rather than shipping a link that looks filtered and is not.
- **`window.open` must be called synchronously from the click** — a call after an `await` is blocked. Where the id is only known after a request, render a link instead of opening a tab. A blocked popup returns `null`: report it with the URL rather than leaving someone waiting, and do not log the card, because the trip never happened.
- **The card needs `control_ad_id` and `test_ad_id` fields**, or "paste the new id onto the card" is impossible to follow. Accept **either the id or the platform URL** — every route to an id goes through a URL containing it — parse the id out, and *show what was read* rather than silently rewriting the input. Refuse a link naming an ad set or campaign: that id can never match an ad, and taking the number anyway puts an id on the card that looks comparable and is not. Refuse a legacy `<id>_V2` by name. Normalize at save time as well as on input, since a link can be pasted and Save clicked without the field ever losing focus — and when a field holds something invalid, **block the save** and say why; dropping it silently leaves the old value while looking like it saved.
...
- **Pause button** (every row, always visible): confirms, opens the native editor for that entity, and logs a Completed card. It does not pause anything itself — see the Changelog spec above.
...
- Duplicate and Pause change nothing themselves: they open the platform's own editor and log what was intended. Confirm first anyway, so nobody lands in Ads Manager without knowing why. A dashboard button that spends money on a mis-click is not a feature, and the surest way to never build one is to keep the credential out of the page.
...
- Click Duplicate and confirm **nothing is created**: no test ad id appears on the card, no row is added to the creative table, and the control ad's spend is unchanged. The card should read *awaiting an id*. Switching the Creatives page to L30 must leave the card numbers unchanged.
- Click Pause and confirm it opens the editor, logs one Completed card, and sets no local paused flag. Click it twice in a day and confirm one card, not two.
- Assert the generated ads-manager URL **byte-for-byte** against a URL the platform actually produced. Do not test by loading it — a sandbox usually cannot reach the platform's domain, so assert the URL the page requested. Stub the popup (`window.open`) rather than opening real tabs.
- Paste an ad-set link, a campaign link and a legacy `<id>_V2` into the ad-id fields. Each must be refused with a reason, and the save must be **blocked** rather than the value silently dropped.
