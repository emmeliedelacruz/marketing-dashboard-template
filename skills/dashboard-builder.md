...
- 2026-08-23 (latest): The perf strip auto-populates from the unit's ads once a card is past To Test, so a card moved into Testing by hand is never blank. Explicit ad IDs from Duplicate still take precedence.
...
- Every Changelog card with a test resolves both ad IDs against the creative data, or is honestly labelled a snapshot. Move a card into Testing by hand and confirm its stats appear with no ad ID typed. Switching the Creatives page to L30 must leave the card numbers unchanged.
