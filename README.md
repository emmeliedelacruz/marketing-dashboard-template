...
One HTML file — no build step. Open it in a browser to look at it, then either publish it as an AI link or put it behind a small server if you need a shared board and live data. See [Where to run it](#where-to-run-it).
...
3. **Sheet → HTML.** The refresh job regenerates the data arrays and republishes. On an AI link this is the only option — a published page can't reach outside itself — and on a server you'd read the database directly instead. Either way nothing on this dashboard is intraday, so a live read buys nothing a daily refresh doesn't. See [Where to run it](#where-to-run-it).
